 ...
 * @author mzinal
 */
public class LockerPerfDemo implements Runnable {

    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(LockerPerfDemo.class);
    ...
    public LockerPerfDemo(PessimisticLocker locker, int number) {
        this.locker = locker;
        this.number = number;
    ...
    @Override
    public void run() {
        final LockerOwner owner = new LockerOwner("perf-demo", String.valueOf(number));
        final List<LockerItem> items = new ArrayList<>();
        locker.unlock(owner);
        ...
        for (int step = 0; step < THREAD_STEPS; ++step) {
            ...
            for (int itemNo = 0; itemNo < itemCount; ++itemNo) {
                ...
                items.add(new LockerItem(accountDt, accountKt));
            }
            LockerResponse response;
            do {
    ...
    public static void main(String[] args) {
        ...
        */
        try (PessimisticLocker locker = new LockerClient("http://localhost:8080/v1")) {
            System.out.println("Connected!");
            ArrayList<Thread> threads = new ArrayList<>(THREAD_COUNT);
            for (int threadNum = 0; threadNum < THREAD_COUNT; ++threadNum) {
                Thread t = new Thread(new LockerPerfDemo(locker, threadNum));
                t.setDaemon(true);
